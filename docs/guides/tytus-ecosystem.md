...
Last updated: 2026-05-03. Applies to `tytus` v0.6.9.
...
Download `tytus-windows-x86_64.zip` from the GitHub release or use the installer with `TYTUS_USE_RELEASE=1` when instructed. CLI/MCP packaging is present in v0.6.9; full daemon/tray/tunnel runtime is not yet production-complete on Windows.
