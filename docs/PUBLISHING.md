...
```
tytus-cli repo
├── install.sh          ← mastered here
├── install.ps1         ← mastered here
├── web/
│   ├── index.html      ← landing page
│   ├── build.sh        ← Cloudflare Pages build step
│   ├── _redirects      ← CF routing
...
└── contrib/homebrew/
    └── tytus.rb        ← formula template with {{VERSION}}, {{SHA_*}} placeholders

Cloudflare Pages project
├── Watches: main branch of tytus-cli
├── Build command: sh web/build.sh
├── Build output:  web/dist
└── Serves install.sh / install.ps1 / index.html from edge cache
...
1. Go to Cloudflare dashboard → Workers & Pages → Create Project → Connect to Git
2. Select repo: `traylinx/tytus-cli`
3. Production branch: `main`
4. Build command: `sh web/build.sh`
5. Build output directory: `web/dist`
6. Save
7. Custom domain: add `tytus.traylinx.com` (CNAME to the pages.dev subdomain)
