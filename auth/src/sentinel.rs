...
pub async fn fetch_wannolot_pass(
    ...
    let body: WannolotPassResponse = resp.json().await.map_err(|e| {
        AtomekError::Other(format!("Failed to parse wannolot-pass response: {}", e))
    })?;

    if !body.has_pass {
        let msg = body
            .message
            .unwrap_or_else(|| "No Tytus subscription".into());
