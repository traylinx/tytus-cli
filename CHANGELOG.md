...
- Updates local TytusOS docs for the new Atomek Ask pod transport.
...
- Publishes updated public beta download/install surfaces for the beta40 release.
...
- Pins the Tytus app catalog to version 47 so Atomek `0.4.15` launches as Mission Control by default with Control Tower resource/mission orchestration UX.
...
- Pins the Tytus app catalog to version 44 so Atomek `0.4.13` and JULI3TA `0.3.14` resolve from the same immutable catalog snapshot.
- Refreshes vendored TytusOS dist and public beta installer/download references.
...
- JULI3TA creator helper buttons now run operation-scoped work: Inspire, Suggest, Polish, AI Optimize, and Cover generation no longer abort each other; Create/Restyle still cancels draft helpers to freeze the submitted song snapshot.

## v0.6.14-beta.21 - 2026-05-10

- Bundled TytusOS dist refreshed from `tytus-os` `v1.0.14-apps-platform`, including JULI3TA 0.3.10 fallback/catalog pins.
- JULI3TA Restyle Inspire/Suggest now skips the broken `minimax/ail-compound` chat alias and treats provider-incompatible 400s as retryable model fallbacks instead of user-visible hard failures.

## v0.6.14-beta.20 - 2026-05-10

- Bundled TytusOS dist refreshed from `tytus-os` `v1.0.13-apps-platform`, including Atomek 0.4.12 fallback/catalog pins.
- Atomek release adds responsive chat hardening, wide markdown table/code overflow handling, cleaner open-editor rows, and an embedded Agents dock for local Claude/pi/OpenCode/Codex-style runs via the Tytus host bridge.

## [Unreleased]

### Added
- Tray local bridge can now launch supervised local jobs and stream job events for TytusOS/Atomek agent workflows.
- Bundled TytusOS dist refreshed from `tytus-os` `v1.0.8-apps-platform`, including JULI3TA 0.3.9 and Atomek 0.4.7 fallback/catalog pins.
